...
if($num == 0){
    ...
    while($riga = $risultato->fetch(PDO::FETCH_ASSOC)){
                $luogo = "SELECT * FROM luoghi WHERE IDLuogo = ".$riga['CodLuogo'];
                $risultato2 = $connessione->query($luogo);
                $riga2 = $risultato2->fetch(PDO::FETCH_ASSOC);
                ...
                .'<div class="container">'
                    ...
                    <div class="form-group">
                        ...
                        echo '<script>document.getElementById("provincia2").value = "'.$riga['provincia'].'"; </script>
                        <div class="form-group">
                        <label for="provincia2">Provincia:</label>
                        <select class="form-control" id="provincia2" name="provincia2" style="width: 50%;" value="'.$riga2['provincia'].'">';
                        while($riga3 = $risultato->fetch(PDO::FETCH_ASSOC)){
                            echo '<option value="'.$riga3['nome'].'">'.$riga3['nome'].'</option>';
                        }
                        echo '</select></div>';
                        
                      echo '<div class="form-group">
                        <label for="città2">Città:</label>
                        <input style="width: 50%;" class="form-control" name="città2" id="città2" value="'.$riga2['citta'].'" required="true">
                      </div>
                      
                      <script> document.getElementById("tipologia2").value = "'.$riga['tipologia'].'"; </script>
                        <div class="form-group">
                            <label for="tipologia2">Tipologia:</label>
                            <select class="form-control"  name="tipologia2" id="tipologia2" style="width: 50%;">
